...
int Board::pieceAtLoc(int x, int y) {
	...
	while (piece<6 && check == 0) {
		if (list[piece].getX() == x && list[piece].getY() == y) {
			if ((status == -1 && list[piece].getT() == 2) || (status == 1 && (list[piece].getT() == 1 || list[piece].getT() == 5))) {
...
int Board::validMove() {
	for (int piece = 0; piece < 6; piece++) {
		if (list[piece].getX() == x1 && list[piece].getY() == y1) {
			if (x2 - x1 == 1 || x2 - x1 == -1) {
				if (list[piece].getT() == 1 || list[piece].getT() == 5) {
					if ((y2 - y1 == -1 || y2 - y1 == 1)) {
						return 1;
					}
					else {
...
void Board::move() {
	...
	for (int select = 0; select < 7; select++) {
		if (list[select].getX() == x1 && list[select].getY() == y1) {
			...
			if (list[select].getT() == 1 && list[select].getX() == Warp.getX() && list[select].getY() == Warp.getY()) {
				gotoxy(15,0);
				cout << "Use warp? y/n: ";
				cin >> orNah;
				if (orNah == 'n') {
					if (list[select].getT() == 5) {
						gotoxy(15, 10);
						cout << "Use bone? y/n: ";
						cin >> orNah;
						if (orNah == 'y') {
							int houndx, houndy, inList = 0;
							while (inList == 0) {
								gotoxy(15, 11);
								cout << "Bone to x: ";
								cin >> houndx;
								...
								for (int piece = 0; piece < 4; piece++) {
									if (houndx == list[piece].getX() && houndy == list[piece].getY()) {
										inList = 1;
										list[piece].out();
										gotoxy(15, 20);
										cout << "hound down";
					...
					srand(time(NULL));
					int r1, r2;
					while ((r1+r2)%2 == 0 || spaceEmpty(r1, r2) == 0) {
						r1 = rand() % 8 + 1; //range of numbers between 1 and 20.
						r2 = rand() % 8 + 1; //range of numbers between 1 and 20.
					}
					list[select].set(r1, r2);
				}
			}
			else {
				if (list[select].getT() == 5) {
					...
					if (orNah == 'y') {
						...
						while (inList == 0) {
							...
							cin >> houndy;
							for (int piece = 0; piece < 6; piece++) {
								if (houndx == list[piece].getX() && houndy == list[piece].getY() && list[piece].getT() == 2) {
									inList = 1;
									list[piece].out();
									gotoxy(25, 20);
									cout << "hound down";
			...
			if (list[select].getT() == 1 && list[select].getX() == Bone.getX() && list[select].getY() == Bone.getY() && Bone.getA() == 1) {
				list[select].gotBone();
				gotoxy(0, 16);
				cout << "list[" << select << "] got bone";
				Bone.out();
			}
			else if (list[select].getT() == 2 && list[select].getX() == Bone.getX() && list[select].getY() == Bone.getY() && Bone.getA() == 1) {
				list[select].out();
				Bone.out();
				gotoxy(0, 16);
				cout << "hound down";
