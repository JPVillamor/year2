//JP Villamor 3A Jan 2018
#include <H:/myLib.h>
using namespace std;
ifstream myinfile ("S://Public/CS/ACSLinput/srSample.txt");
...
int main () {
	...
	for (int x = 0; x < inputs.size(); x++) {
        int braces = 0, brackets = 0, parenth = 0, position = 0;
        if (inputs[x].find("{") == -1) {
            braces = 1;
        }
        if (inputs[x].find("}") == -1) {
            ...
            while (position <= inputs[x].length()) {
                ...
                if (position > inputs[x].find("]")) {
                    if (ops.find(inputs[x][position]) != -1 || position == inputs[x].length()) {
                        newstr.insert(position, "}");
                        cout << newstr << "    " << position+1 << endl;
                    }
                }
                position++;
            }
        ...
        if (inputs[x].find("[") == -1) {
            ...
            while (position < inputs[x].length()) {
                string newstr = inputs[x];
                if (!(position < inputs[x].find(")") && position > inputs[x].find("(")) && position < inputs[x].find("]") && position > inputs[x].find("{")) {
        ...
        if (inputs[x].find("]") == -1) {
            ...
            while (position <= inputs[x].length()) {
                ...
                if (!(position < inputs[x].find(")") && position > inputs[x].find("(")) && position > inputs[x].find("[")) {
                    if (ops.find(inputs[x][position]) != -1 || ((inputs[x][position] == '}') && nums.find(inputs[x][position-1]) != -1)) {
                        newstr.insert(position, "]");
        ...
        if (inputs[x].find("(") == -1) {
            ...
            while (position < inputs[x].length()) {
                string newstr = inputs[x];
                if (position > inputs[x].find("[") && position < inputs[x].find(")")) {
